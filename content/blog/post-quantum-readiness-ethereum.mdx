...
This is the first article in PQBEAT, so it should do one useful thing: explain what we mean when we talk about Ethereum's post-quantum readiness.
...
That is also why there is only one public article right now. A single clear note is more useful than three posts saying roughly the same thing in different ways. We can add more once there is something genuinely distinct to say.
